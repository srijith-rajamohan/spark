...
public  class ShufflePartitionsUtil {
   ...
   * @return (undocumented)
   */
  static   int[] splitSizeListByTargetSize (scala.collection.Seq<java.lang.Object> sizes, long targetSize)  { throw new RuntimeException(); }
  /**
   * Splits the skewed partition based on the map size and the target partition size
   * after split, and create a list of <code>PartialMapperPartitionSpec</code>. Returns None if can't split.
   * @param shuffleId (undocumented)
   * @param reducerId (undocumented)
   * @param targetSize (undocumented)
   * @return (undocumented)
   */
  static public  scala.Option<scala.collection.Seq<org.apache.spark.sql.execution.PartialReducerPartitionSpec>> createSkewPartitionSpecs (int shuffleId, int reducerId, long targetSize)  { throw new RuntimeException(); }
