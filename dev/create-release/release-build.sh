...
if [[ "$1" == "finalize" ]]; then
  ...
  FEATURE_VERSION=$(echo $RELEASE_VERSION | cut -d. -f2)
  MAINTENANCE_VERSION=$(echo $RELEASE_VERSION | cut -d. -f3)
  if [[ "$MAINTENANCE_VERSION" != "0" ]]; then
    PREVIOUS_VERSION="$MAJOR_VERSION.$FEATURE_VERSION.$((MAINTENANCE_VERSION - 1))"
    svn rm --username $ASF_USERNAME --password "$ASF_PASSWORD" --force -m"Remove old release" \
      "$RELEASE_LOCATION/spark-$PREVIOUS_VERSION"
    echo "spark-$PREVIOUS_VERSION has been removed from the Mirror Network"
  fi
